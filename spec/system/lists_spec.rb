...
describe '投稿のテスト' do
  ...
  describe '投稿画面のテスト' do
    before do
      vsit todolists_new_path
    end
    contex '表示の確認 ' do
      it 'todolists_new_pathが"/todolists/new"であるか' do
        expet(current_path).to eq('/todolists/new')
      end
      ...
      contex '投稿処理のテスト' do
        it '投稿後のリダイレクト先は正しいか' do
          fill_in 'list[title]', with: Faker::Lorem.characters(unmber:5)
          fill_in 'list[body]', with: Faker::Lorem.characters(unmber:20)
          click_button '投稿'
    ...
    describe '詳細画面のテスト' do
      ...
      contex 'リンクの遷移先の確認' do
        it '編集の遷移先は編集画面か' do
          edit_link = find_all('a')[0]
          edit_link.click
          expect(current_path).to eq('/todolists/' + list.idto_s + '/edit')
        end
      end
      contex 'list削除のテスト' do
        it 'listの削除' do
